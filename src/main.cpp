          ...
          // j[1] is the data JSON object
            ...
            if(too_close){
                ...
                for(int j = 0; j< sensor_fusion.size();j++)
                {
                    ...
                    if((left_lane>-1)&&(d<(2+4*left_lane+2))&&(d>(2+4*left_lane -2)))
                    {
                    ...
                    else if((right_lane<4)&&(d<(2+4*right_lane+2))&&(2+4*left_lane-2))
                    {
                        double vx = sensor_fusion[j][3];
                        double vy = sensor_fusion[j][4];
                        double check_speed = sqrt(vx*vx+vy*vy);
                ...
                if(left_disable == false&&lane>0)
                    lane = left_lane;
                else if(right_disable==false&&lane<3)
                    lane = right_lane;
                else
                    ref_vel -=.224;
